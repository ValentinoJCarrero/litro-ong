...
export class MailerService {
  ...
  async sendNewsletterMail(
    ...
    try {
      ...
      const mailList = users.data.map(
        (user) => user.isSubscribed === true && user.email,
      );
  ...
  async birthdayGreetings(): Promise<void> {
    const todayBirthdateUsers =
      await this.usersRepository.getBirthdayTodayUsers();
