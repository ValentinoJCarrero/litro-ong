  ...
  async sendNewsletterMail(title, subtitle, description, primaryImage?): Promise<void> {
    try {
        ...
        const msg = {
          ...
          html:  `<div style="font-family: Arial, sans-serif; text-align: center; color: #333;"><h1 style="font-size: 24px; font-weight: bold; margin-bottom: 20px;">${title}</h1><h2 style="font-size: 20px; font-weight: normal; margin-bottom: 20px;">${subtitle}</h2><p style="font-size: 16px; margin-bottom: 20px;">${description}</p>${primaryImage ? `<img src="${primaryImage}" alt="Newsletter Image" style="max-width: 100%; height: auto; margin-bottom: 20px;">` : ''}<img src="https://res.cloudinary.com/dsiic5ax7/image/upload/v1716153635/logo_s6phc5.png" alt="Litro de leche" width="50" height="50"></div>`,};
